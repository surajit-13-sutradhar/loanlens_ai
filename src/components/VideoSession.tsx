...
const PLAN_LABEL_COLOR: Record<string, string> = {
  Safe: "text-emerald-400", Balanced: "text-blue-400", Max: "text-amber-400",
};
...
const PLAN_DOT: Record<string, string> = {
  Safe: "bg-emerald-500", Balanced: "bg-blue-500", Max: "bg-amber-500",
};
const FOIR_COLOR: Record<string, string> = {
  safe: "bg-emerald-400", moderate: "bg-amber-400", risky: "bg-red-400",
};
...
function StatusBadge({ label, status }: { label: string; status: "granted" | "denied" | "pending" | "idle" }) {
  ...
  const dots: Record<string, string> = {
    granted: "bg-emerald-400", denied: "bg-red-400",
    pending: "bg-yellow-400 animate-pulse", idle: "bg-white/20",
  };
  return (
...
function LoanOfferCard({ option, selectedPlan, onSelect }: { option: LoanOption; selectedPlan: string | null; onSelect: (opt: LoanOption) => void }) {
  const isSelected = selectedPlan === option.plan;
  return (
    <div
      onClick={() => onSelect(option)}
      className={`relative group p-6 rounded-2xl border transition-all cursor-pointer overflow-hidden ${PLAN_ACCENT[option.plan]} ${
        isSelected ? `bg-white/10 ${PLAN_SELECTED_RING[option.plan]}` : "bg-white/5 border-white/10"
      }`}
    >
      <div className={`absolute top-5 left-5 w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center ${
        isSelected ? "border-current" : "border-white/20 group-hover:border-white/40"
      }`}>
        <div className={`w-2.5 h-2.5 rounded-full transition-all ${isSelected ? PLAN_DOT[option.plan] : "bg-transparent"}`} />
      </div>
      <div className="flex justify-end items-center mb-8 pl-8">
        <span className={`text-[11px] font-bold uppercase tracking-widest ${PLAN_LABEL_COLOR[option.plan]}`}>{option.plan} Plan</span>
        <span className="text-xs font-bold text-white/50 bg-white/5 border border-white/10 px-3 py-1 rounded-md ml-3">{option.annualInterestRate}% APR</span>
      </div>
      <p className="text-3xl font-mono font-bold text-white mb-2">{inr(option.principalAmount)}</p>
      <p className="text-white/50 text-xs mb-2">
        Pay <span className="text-white/80 font-mono font-medium">{inr(option.monthlyEmi)}</span> per month over{" "}
        <span className="text-white/80 font-medium">{option.tenureMonths} months</span>.
      </p>
    </div>
  );
}
...
export default function VideoSession({ sessionId }: { sessionId: string }) {
  ...
  const chatScrollRef     = useRef<HTMLDivElement>(null);  // ← scroll container
  ...
  const { transcript, isListening, isTranscribing, startListening, stopListening, clearTranscript } = useTranscription({ silenceThreshold: 0.012, silenceDurationMs: 1400 });
  ...
  const isApproved = isComplete && (decision?.decision === "APPROVE" || decision?.decision === "REVIEW");
  ...
  useEffect(() => {
    if (sessionState === "active" && videoRef.current && videoStream) {
      const t = setTimeout(() => { if (videoRef.current) startMonitoring(videoRef.current); }, 1500);
  ...
  }, [sessionState, isVideoVerified, isComplete, isBotTyping, isListening, audioStream, startListening]);

  // ── STT → input box ──────────────────────────────────────────────────────────
  useEffect(() => {
    if (!isListening || isBotTyping || transcript.length === 0) return;
    const latest = transcript[transcript.length - 1].text;
    setChatInput((prev) => { const t = prev.trim(); return t ? `${t} ${latest}` : latest; });
    inputRef.current?.focus();
  }, [transcript, isListening, isBotTyping]);

  // ── Smart scroll — only scrolls if user is already within 120px of the bottom ─
  ...
  useEffect(() => {
    scrollChatToBottom();
  }, [messages, isBotTyping, scrollChatToBottom]);

  // ── TTS: speak every new bot message ─────────────────────────────────────────
  useEffect(() => {
    if (messages.length === 0) return;
    const last = messages[messages.length - 1];
    if (last.role === "bot") {
      const t = setTimeout(() => speak(last.text), 100);
      return () => clearTimeout(t);
    }
  }, [messages, speak]);
  ...
  const triggerBotResponse = useCallback(async (currentMessages: Message[]) => {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({
          messages: currentMessages.map((m) => ({ role: m.role === "bot" ? "assistant" : "user", content: m.text })),
      ...
      if (data.isComplete) {
        ...
        if (data.loanDecision) {
          ...
          if (data.loanDecision.decision !== "REJECT" && data.loanDecision.loanOptions?.length) {
  ...
  const processUserResponse = async (userText: string) => {
    ...
    // Scroll the chat panel (not the page) to bottom
  ...
  const handleSelection = async (offer: LoanOption | null, status: "accepted" | "rejected") => {
    ...
    try {
      await fetch("/api/sessions/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: sessionId, kycData: formData, loanDecision: { selectedOffer: offer, rawDecision: decision }, status: "submitted" }),
      });
    } catch (err) {
  ...
  }, [stopListening, stopAll, stopMonitoring, stopTTS]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {isConsentOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-[#121212] border border-white/10 rounded-3xl max-w-md w-full p-8 shadow-2xl">
            ...
            <p className="text-white/60 text-sm leading-relaxed mb-4">To proceed with your loan application, we need to record a short video session for identity verification. By clicking "I Agree," you consent to:</p>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={async () => { setIsConsentOpen(false); await handleStart(); runVerification(); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-4 rounded-2xl transition-all active:scale-95">
                I Agree — Start Session
              </button>
              <button onClick={() => setIsConsentOpen(false)} className="w-full bg-white/5 hover:bg-white/10 text-white/40 text-sm py-3 rounded-2xl transition-all">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl">
          <div className="bg-[#1c1c1e] border border-white/10 w-full max-w-5xl rounded-[32px] overflow-hidden shadow-2xl p-10">
            <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
              <div>
                <h2 className="text-3xl font-bold text-white mb-1">Exclusive Offers For You</h2>
                <p className="text-white/50 text-sm">Select a plan to proceed with documentation.</p>
              </div>
              {decision && (
                <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-5 py-3">
                  <div className={`w-2 h-2 rounded-full animate-pulse ${FOIR_COLOR[decision.foirStatus]}`} />
                  <div className="text-sm font-medium">
                    <span className="text-white/40">FOIR</span>
                    <span className={`ml-2 font-mono ${FOIR_TEXT_COLOR[decision.foirStatus]}`}>{decision.foir.toFixed(1)}% — {decision.foirStatus.toUpperCase()}</span>
                  </div>
                  <span className="text-xs text-white/30 font-mono border-l border-white/10 pl-3">Score: {decision.proxyScore}</span>
                </div>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
              {offers.map((opt) => <LoanOfferCard key={opt.plan} option={opt} selectedPlan={selectedOffer?.plan ?? null} onSelect={setSelectedOffer} />)}
            </div>
            <div className="flex items-center justify-end gap-4 border-t border-white/10 pt-8">
              <button onClick={() => handleSelection(null, "rejected")} className="bg-white/5 hover:bg-white/10 text-white/40 hover:text-white/80 transition-colors text-sm font-medium rounded-xl px-8 py-3.5">
                Decline Offers
              </button>
              <button
                disabled={selectedOffer === null}
                onClick={() => selectedOffer && handleSelection(selectedOffer, "accepted")}
              ...
              >
                {selectedOffer === null ? "Select an Offer to Accept" : (
                  <>Accept <span className="text-green-200 font-mono group-hover:scale-105 transition-transform">{inr(selectedOffer.principalAmount)}</span> Offer</>
                )}
              </button>
            </div>
          </div>
      ...
      {sessionState === "completed" && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
          <div className="bg-[#121212] border border-white/10 rounded-3xl max-w-sm w-full p-10 text-center shadow-2xl">
            ...
            <p className="text-white/50 text-sm leading-relaxed mb-8">Your loan application has been received. Our team will review your KYC details and reach out within 24–48 hours.</p>
            <button
              onClick={() => { setSessionState("idle"); setIsComplete(false); setDecision(null); setOffers(null); setSelectedOffer(null); setMessages([]); setFormData({}); }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {isSpeaking       && <StatusBadge label="Speaking…"           status="granted" />}
        {verificationStep === "analyzing" && <StatusBadge label="Verifying Identity…" status="pending" />}
        {sessionState === "active" && faceStats.warning && <StatusBadge label={faceStats.warning} status="denied" />}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-4 items-start">
        ...
        <div className="flex flex-col gap-4">
          <div className="relative rounded-2xl overflow-hidden bg-white/5 border border-white/10 aspect-video">
            <video ref={videoRef} autoPlay muted playsInline className={`w-full h-full object-cover transition-opacity duration-500 ${sessionState === "active" ? "opacity-100" : "opacity-0"}`} />
            {sessionState !== "active" && (
            ...
            {sessionState === "active" && verificationStep === "analyzing" && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center z-30 p-6 text-center">
                ...
                {faceStats.warning && <p className="mt-4 text-red-400 text-xs font-medium bg-red-500/10 px-3 py-1.5 rounded-full border border-red-500/20">{faceStats.warning}</p>}
                {detectedAge && <p className="mt-3 text-emerald-400 text-xs font-mono">Age group: {detectedAge}</p>}
              </div>
            )}
            ...
            {sessionState === "active" && verificationStep !== "analyzing" && faceStats.warning && (
              <div className="absolute bottom-14 left-0 right-0 flex justify-center z-20">
                <div className="bg-red-500/90 backdrop-blur-sm text-white text-xs font-medium px-4 py-2 rounded-xl">{faceStats.warning}</div>
              </div>
            )}
          ...
          <div className="flex items-center gap-3">
            {sessionState === "idle" && (
              <button onClick={() => setIsConsentOpen(true)} className="bg-blue-600 hover:bg-blue-500 active:scale-95 transition-all text-white text-sm font-medium rounded-xl px-6 py-2.5">
                Start Session
              </button>
            )}
            {sessionState === "active" && (
              <>
                ...
                {isSpeaking && (
                  <button onClick={stopTTS} className="bg-white/10 hover:bg-white/15 active:scale-95 transition-all text-white/60 hover:text-white text-sm font-medium rounded-xl px-4 py-2.5 flex items-center gap-2">
            ...
            {sessionState === "stopped" && (
              <>
                <button onClick={handleNewSession} className="bg-white/10 hover:bg-white/15 active:scale-95 transition-all text-white text-sm font-medium rounded-xl px-6 py-2.5">New Session</button>
                {downloadUrl && (
          ...
          >
            {isRejected && (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 gap-4">
                ...
                {decision && <p className="text-[10px] text-white/20 font-mono">Proxy score: {decision.proxyScore} · FOIR: {decision.foir.toFixed(1)}%</p>}
              </div>
            )}
